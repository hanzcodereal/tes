...
function getRunsText(r) { 
    return Array.isArray(r) ? r.map(x=>x.text||'').join('') : ''; 
}

function removeKeysRecursive(o, k) { 
    if(!o||typeof o!=='object')return; 
    if(Array.isArray(o)){o.forEach(i=>removeKeysRecursive(i,k));return;} 
    for(const key of Object.keys(o)){
        if(k.includes(key))delete o[key];
        else if(typeof o[key]==='object')removeKeysRecursive(o[key],k);
    } 
}
                    ...
                    if (sec.musicShelfRenderer) {
                        for (const item of sec.musicShelfRenderer.contents || []) {
                            if (item.musicResponsiveListItemRenderer) {
                                ...
                                topSongs.push({ 
                                    videoId: i.playlistItemData?.videoId||'', 
                                    ...
                                    thumbnails: i.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails||[] 
                                });
                            }
                        }
                    }
