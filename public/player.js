...
const S={ht:[],sr:[],ar:[],sq:'',filter:'all',ct:null,pl:[],pi:-1,ps:'',ip:false,il:false,rm:'off',autoNext:true,yp:null,yr:false,iv:null,pt:0,pd:0,at:'home',ld:{type:'none',lines:[]},cli:-1,lo:false,lyricsLoaded:false,server:'1',lyricsFetching:false,playbackRate:1.0,sleepSecondsLeft:0,sleepEndWithTrack:false};
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: '🎵 Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di HanzzMusify!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
