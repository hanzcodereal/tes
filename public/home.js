        ...
        try {
            ...
            for (const q of queries) {
                ...
                if (d.status && d.result.songs) {
                    const songs = d.result.songs.map(function(s) {
                        return {
                            id: s.videoId,
                            videoId: s.videoId,
                            title: cn(s.title),
            ...
            if (allSongs.length > 0) {
                const categorySongs = {
                    'home-grid': allSongs.slice(0, 6),
                    'home-new': allSongs.slice(6, 12),
                    'home-popular': allSongs.slice(12, 18),
                    'home-tiktok': allSongs.slice(18, 24),
                    'home-galau': allSongs.slice(24, 30),
                    'home-hits': allSongs.slice(30, 36)
                };
                ...
                for (const [id, songs] of Object.entries(categorySongs)) {
                    const container = gid(id);
                    if (container && songs.length > 0) {
                        const idx = categoryIndex[id] || 0;
                        if (id === 'home-grid') {
                            container.innerHTML = songs.map(function(t, i) {
                                return '<div onclick="PK(\'home' + idx + '\',' + i + ')" class="bg-[#181818] hover:bg-[#282828] rounded-xl flex items-center gap-3 p-2 cursor-pointer active:scale-95 transition-colors" style="animation-delay:' + (i * 50) + 'ms"><img src="' + getHDImage(t.cover) + '" class="w-14 h-14 rounded-lg object-cover shadow-lg" onerror="this.src=\'' + FI + '\'" /><span class="font-bold text-sm line-clamp-2">' + es(t.title) + '</span></div>';
                            }).join('');
                        } else {
                            container.innerHTML = songs.map(function(t, i) {
