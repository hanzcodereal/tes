const Artist={
    ...
    open(id,name){
        ...
        fetch(API.artist+'?id='+id).then(function(r){return r.json();}).then(function(d){
            if(d.status&&d.result){
                var a=d.result;
                var headerImg=a.thumbnails&&a.thumbnails[2]?getHDImage(a.thumbnails[2].url):FI;
                var html='';
                ...
                if(a.topSongs&&a.topSongs.length>0){
                    ...
                    a.topSongs.slice(0,10).forEach(function(s,i){
                        var im=s.thumbnails&&s.thumbnails[0]?getHDImage(s.thumbnails[0].url):FI;
                        html+=`
                ...
                if(a.topAlbums&&a.topAlbums.length>0){
                    ...
                    a.topAlbums.forEach(function(al){
                        var im=al.thumbnails&&al.thumbnails[1]?getHDImage(al.thumbnails[1].url):FI;
                        html+=`
                ...
                if(a.topSingles&&a.topSingles.length>0){
                    ...
                    a.topSingles.forEach(function(sg){
                        var im=sg.thumbnails&&sg.thumbnails[1]?getHDImage(sg.thumbnails[1].url):FI;
                        html+=`
                ...
                if(a.topVideos&&a.topVideos.length>0){
                    ...
                    a.topVideos.forEach(function(vd){
                        var im=vd.thumbnails&&vd.thumbnails[1]?getHDImage(vd.thumbnails[1].url):FI;
                        html+=`
                ...
                if(a.featuredOn&&a.featuredOn.length>0){
                    ...
                    a.featuredOn.forEach(function(fo){
                        var im=fo.thumbnails&&fo.thumbnails[1]?getHDImage(fo.thumbnails[1].url):FI;
                        html+=`
                ...
                if(a.similarArtists&&a.similarArtists.length>0){
                    ...
                    a.similarArtists.forEach(function(s){
                        var im=s.thumbnails&&s.thumbnails[1]?getHDImage(s.thumbnails[1].url):FI;
                        html+=`
    ...
    play(vid,title,artist){
        ...
        S.ps='artist';S.pl=[S.ct];S.pi=0;S.lyricsLoaded=false;S.ld={type:'none',lines:[]};S.cli=-1;S.lyricsFetching=false;UU();MP.show();S.il=true;UB();
        if(S.yp&&S.yr&&vid)S.yp.loadVideoById({videoId:vid});
        if(S.lo) fetchLyrics(vid);
    }
};